...
## Key Skills
`JOIN` · `CASE WHEN` conditional aggregation · Funnel analysis with correlated subqueries · `STRING_AGG ... FILTER` · Date-range `JOIN` for campaign attribution · `CTE`-based table construction
...
## Hardest Query
The `campaign_analysis` table build: combining a visit-level `GROUP BY` (with multiple conditional `SUM`s for different event types) with a **date-range join** to a separate campaign table, while also aggregating an ordered, filtered product list in the same pass — all in a single CTE.
